...
def process_order():
    ...
    try:
        ...
        # System prompt (truncated for brevity - use your original one)
        system_prompt = """You are a food ordering assistant for a restaurant.
Menu: Chicken Sandwich ($5), Fries ($2), Coke ($1.5), Veggie Burger ($4), Pizza Slice ($3)

Determine intent and respond with JSON only:
{
  "intent": "order_food|add_item|remove_item|update_quantity|ask_menu|finalize_order",
  "items": [{"name": "Chicken Sandwich", "qty": 2, "price": 10}],
  "total": 10.0
}
...
# Add at the end of your app.py file, replace the existing if __name__ == '__main__':
